...
`opensabre-starter-governance` 从 `0.4.0` 开始提供审计日志和限次能力，`0.7.0` 增加错误码目录与字典组件。业务应用只需要引入 starter 并添加注解，具体数据入库、规则管理和计数逻辑统一由 sysadmin 承担。
...
```groovy
implementation 'io.github.opensabre:opensabre-starter-governance:0.7.0'
```
